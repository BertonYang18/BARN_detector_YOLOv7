...
def label2img_paths(img_paths):
    ...
    if 'train' in img_paths[0]:
        sa, sb = os.sep + 'annotations' + os.sep + 'train' + os.sep, os.sep + 'all_frames' + os.sep  # /images/, /labels/ substrings
    else:
        sa, sb = os.sep + 'annotations' + os.sep + 'test' + os.sep, os.sep + 'all_frames' + os.sep
    ...
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            for p in path if isinstance(path, list) else [path]:
                p = p.replace('frames', 'annotations')
                ...
                if p.is_dir():  # dir
                    ...
                    with open(p, 'r') as t:
                        ...
                        parent = str(p.parent) + os.sep + 'frames' + os.sep
                        # f += [x.replace('./', parent)  if x.startswith('./') else x for x in t]  # local to global path
                        f += [parent + x + '.jpg' for x in t]
            ...
            # self.img_files = sorted([x.replace('/', os.sep) for x in f if x.split('.')[-1].lower() in img_formats])
            # key_frame_file = []
            # for my_oneimg in self.img_files:
            #     img_num = int(my_oneimg.split(os.sep)[-1].split('_')[-1].split('.')[0])
            #     if img_num % 10 == 0:
            #         key_frame_file.append(my_oneimg)
            # self.img_files = key_frame_file

            # self.img_files = sorted([x for x in f if x.suffix[1:].lower() in img_formats])  # pathlib
            # assert self.img_files, f'{prefix}No images found'
            assert self.label_files, f'{prefix}No images found'
        ...
        self.img_files = label2img_paths(self.label_files)
        cache_path = (p if p.is_file() else Path(self.label_files[0]).parent.parent).with_suffix('.cache')  # cached labels
            ...
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    with open(lb_file, 'r') as f:
                        l = [x.split()[:5] for x in f.read().strip().splitlines()]
